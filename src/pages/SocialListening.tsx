...
const SocialListening = () => {
  ...
  const fetchData = async () => {
    try {
      console.log("SocialListening: Fetching data...");
      setLoading(true);
      const data = await getSocialListenings();
      console.log("SocialListening: Data received", data);

      if (!Array.isArray(data)) {
        console.error("SocialListening: Data is not an array:", data);
      ...
      console.error("SocialListening: Fetch error:", err);
  ...
  const handleSaveEdit = async () => {
    if (selectedItem) {
      setEditError(null);
      try {
        const updatedItem = await editSocialListening(selectedItem.id, {
  ...
  const handleDelete = (item: SocialListeningType, index: number) => {
    setSelectedItem(item);
          ...
          <div className="space-y-4">
            ...
            <div className="flex justify-end gap-2 pt-4">
              ...
              <Button
                onClick={async () => {
                  setCreateError(null);
                  const mentionsNum = Number(newForm.mentions) || 0;
                  try {
                    const newItem = await createSocialListening({
                      title: newForm.title,
                      mentions: mentionsNum,
                    });
